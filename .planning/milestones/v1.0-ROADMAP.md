...
**Key Decisions:**
- Vercel serverless for API proxy (zero-config deployment)
- Supabase pgvector for lyrics storage and semantic search
- text-embedding-3-small for embeddings (5x cheaper than ada-002)
- gpt-4o-mini for lyric selection (cost-effective, fast)
- Temperature 0.6 for emotional variation on repeated questions
- Match threshold 0.70 for similarity scoring
- Character-by-character JS typewriter (not CSS) for multi-line lyric support
- React 19 useState (not useActionState) for fine animation control

**Issues Resolved:**
- Vercel/Vite compilation boundary: API functions must be self-contained
- Redis env var naming: REDIS_KV_REST_API_URL/TOKEN from Vercel Marketplace
- React 19 strict hooks: setState in effects must be in timer callbacks
- Inline arrow callbacks causing effect re-triggers in LyricDisplay

**Issues Deferred:**
- Rotate Redis token (credentials exposed during debugging)
- Pre-existing lint errors in api/ask.ts (2x no-explicit-any)
- Empowerment questions may benefit from threshold tuning (0.70 → 0.65)

**Technical Debt Incurred:**
- api/ask.ts is self-contained (~260 lines) due to Vercel compilation boundary
- No vector indexes on lyrics table (acceptable for 30 rows, needs review at scale)
